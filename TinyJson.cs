    ...
    public struct Parser
    {
        ...
        private struct Context
        {
            ...
            public void Error(string reason)
            {
                ...
                byte[] newa = new byte[cursor - outset + 1];
                Array.Copy(input, outset, newa, 0, cursor - outset + 1);
                elog = Encoding.Default.GetString(newa);
                errorline = 0;
                int emark = 0;
                ...
                if (cursor > emark) errorline++;
                estart = Math.Abs(outset - emark - 1) / 2;
            }
        }
        ...
        private bool ParseWord(Words word)
        {
            ...
            if (context.nodeprocs.Count != 0)
            {
                ...
                {
                    context.Error("< ERROR > Canot parse word !  Type: " + word.ToString());
                    return false;
                }
        ...
        private bool DoToken(byte c)
        {
            ...
            else
            {
                ...
                if (token == Tokens.__)
                {
                    context.Error("< ERROR > ASCII Token canot compair Enum Token");
                    return false;
                }
            }
            ...
            if (context.token_state == TokenStates.__)
            {
                ...
                context.Error("< ERROR > Token Stattes Error");
                return false;
            }
            switch (action)
            {
        ...
        private bool ParseToken()
        {
            ...
            {
                context.Error("< ERROR > Token states not OK");
                return false;
            }
            return true;
        }
        ...
        public string Error()
        {
            if (context.errorline == 0 && context.estart == 0)
                return "";
            return "In file : [" + loadpath + "]  " + context.errorline + " : " + context.estart + " \"" + context.elog + "\"\n" + context.error;
        }
        ...
        public Node Load(byte[] input, string path)
        {
            loadpath = path;
            return Load(input);
        }
